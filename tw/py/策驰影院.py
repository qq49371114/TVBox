...
class Spider(Spider):
    ...
    def extract_middle_text(self, text, start_str, end_str, pl, start_index1: str = '', end_index2: str = ''):
                ...
                for i in range(len(plx)):
                    ...
                    for match in matches:
                        ...
                        if 'http' not in match[0]:
                            output += f"#{'📽️天微👉' + match[1]}${number}{xurl}{match[0]}"
                        else:
                            output += f"#{'📽️天微👉' + match[1]}${number}{match[0]}"
                    output = output[1:]
                    purl = purl + output + "$$$"
                purl = purl[:-3]
                return purl
        ...
        if pl == 2:
            ...
            matches = re.findall(start_index1, middle_text)
            if matches:
                new_list = [f'天微👉{item}' for item in matches]
                jg = '$$$'.join(new_list)
                return jg

    def homeContent(self, filter):
        result = {}
        result = {"class": [{"type_id": "77", "type_name": "天微电影🌠"},
                            {"type_id": "78", "type_name": "天微剧集🌠"},
                            {"type_id": "80", "type_name": "天微动漫🌠"},
                            {"type_id": "165", "type_name": "天微短剧🌠"},
                            {"type_id": "166", "type_name": "天微体育🌠"},                                                                         {"type_id": "164", "type_name": "天微4K🌠"},
                            {"type_id": "170", "type_name": "天微演唱会🌠"},                                                                          {"type_id": "79", "type_name": "天微综艺🌠"}],
